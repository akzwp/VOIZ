...
async function run() {
  ...
  check(!/^[a-z]{2}$/i.test(cal.d.querySelector('#calendar_datepick .ui-datepicker-calendar thead th').textContent.trim()), 'Mini weekday header uses Persian letters');
  const fcTitle = cal.d.querySelector('#calendar_main .fc-header h2');
  check(fcTitle && /[\u0600-\u06FF]/.test(fcTitle.textContent), 'FullCalendar month title is Persian: ' + (fcTitle ? fcTitle.textContent : 'missing'));
  const fcDay = cal.d.querySelector('#calendar_main .fc-day-header');
  check(fcDay && /[\u0600-\u06FF]/.test(fcDay.textContent), 'FullCalendar weekday headers are Persian');
  const fcButtons = [...cal.d.querySelectorAll('#calendar_main .fc-header .fc-button')].map(b => b.textContent.trim());
  check(fcButtons.some(t => /[\u0600-\u06FF]/.test(t)), 'FullCalendar nav buttons are Persian');
  check(cal.d.querySelector('#calendar_eventdialog #listenTTS img') && !cal.d.querySelector('#calendar_eventdialog #listenTTS div'), 'TTS speaker icon sits alone inside the anchor (real tpl shape)');
  const dialogWrap = cal.d.querySelector('.voiz-jdate-wrap input[name="date"]');
  check(dialogWrap, 'Event dialog start-date field is wrapped with the Jalali picker');
  check(cal.d.querySelector('.voiz-jdate-wrap input[name="to"]'), 'Event dialog end-date field is wrapped with the Jalali picker');
  const jToggle = cal.d.querySelector('.voiz-jdate-toggle');
  jToggle.click();
  check(!cal.d.querySelector('.voiz-jdate-pop').hidden, 'Jalali date popover opens from its toggle');
  const dayButtons = cal.d.querySelectorAll('.voiz-jdate-pop [data-voiz-jdate="pick"]');
  check(dayButtons.length >= 29, 'Jalali popover shows a full month');
  check(/[۰-۹]/.test(dayButtons[0].textContent), 'Jalali popover digits are Persian');
  dayButtons[4].click();
  cal.d.querySelector('.voiz-jdate-apply').click();
  const gregorian = cal.d.querySelector('.voiz-jdate-wrap input[name="date"]');
  check(/^\d{4}-\d{2}-\d{2}/.test(gregorian.value), 'Picked Jalali date writes back the same Gregorian text format: ' + gregorian.value);
  check(/[۰-۹]/.test(cal.d.querySelector('.voiz-jdate-jalali input').value), 'Jalali mirror field shows Persian digits');
  check(cal.d.querySelector('.voiz-jdate-pop').hidden, 'Popover closes after applying');
  check(cal.d.querySelector('#calendar_eventdialog input[name="event"]'), 'Event dialog keeps module field names');
